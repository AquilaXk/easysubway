...
Android 16 KB page-size gate는 `apps/mobile/release/android-16kb-page-size-gate.json`으로 검증합니다. AAB evidence는 `tools/mobile/check-android-aab-16kb-page-size.sh`로 bundletool config와 native `.so` LOAD segment alignment를 확인하고, runtime evidence는 `tools/mobile/run-android-16kb-page-size-smoke.sh`로 로컬 Android emulator의 `adb shell getconf PAGE_SIZE=16384` 조건을 먼저 확인합니다. 현재 로컬 emulator가 4 KB page-size이면 Go evidence로 쓰지 않고 16 KB system image 또는 Play-installed build 증거를 다시 수집합니다.
