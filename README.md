...
Datapack release readiness gate는 `apps/mobile/release/datapack-release-readiness-gate.json`으로 검증합니다. #1419는 오프라인 데이터팩과 온라인 실시간 overlay를 분리하고, 인접역 graph, 시간표, 시설·pathway, 노선도 좌표, realtime join key, 배포 artifact 검증, ETA 실측 정확도, claim scan 증거가 모두 닫히기 전까지 데이터팩 상용 준비 완료나 실시간 ETA 정확 claim을 막는 P0 release blocker입니다.
