...
//   부산  147/147 (#2068 부산 전면 재설계 2단계: 부전·벡스코 동해선 노드에 각자
//               ordinary 라벨(부전_DH·벡스코_DH)을 오너 SVG에 신규 배치해, 별개
//               station_id로 모델링된 동명 물리역이 각자 자기 라벨을 갖는다 — 이전
//               145에서 +2로 전 역 매치. 두 라벨이 모두 오너 매치이므로 동명 폴백
//               시각 억제(df8d0176) 비대상이 정상이다(좌천 선례: 1호선·동해선 좌천이
//               각자 라벨 매치). 5차 canonicalRules 정합(벡스코(시립미술관)→벡스코,
//               부산역→부산, 경성대·부경대→경성대.부경대, 국제금융센터·부산은행→
//               국제금융센터.부산은행)은 그대로 유지된다.)
