...
const SELF_FILES = ["contracts/documentation/documentation-fragment.json", "contracts/documentation/plan-doc-execution-audit-scope.json", "tools/ci/check-contracts.mjs", "tools/ci/check-contracts.test.mjs", "tools/repo/audit-plan-doc-execution.mjs", "tools/repo/audit-plan-doc-execution.test.mjs"];
const EXPECTED_SCOPE_CANONICAL_SHA256 = "d785db76e79bf88d7d3b971c5d075fc51e588c72a5faab0de90a71a93636b2ba";
...
export function validatePlanDocExecutionScope(scope, errors = []) {
  ...
  if (!Array.isArray(records) || records.length !== 67) return [...errors, "historical inventory count mismatch"];
  const key = (record, field) => `${record?.repository}:${record?.[field]}`;
  if (new Set(records.map((record) => key(record, "prNumber"))).size !== 67) errors.push("duplicate historical record identity");
  if (new Set(records.map((record) => key(record, "mergeSha"))).size !== 67) errors.push("duplicate historical merge identity");
  ...
  if (self?.repository !== "AquilaXk/easysubway" || self?.issueNumber !== 2886 || self?.planOwner !== "PLAN-DOC" || self?.changedPathClass !== "HUB_GOVERNANCE_ONLY" || JSON.stringify(self?.allowedChangedFiles) !== JSON.stringify(SELF_FILES)) errors.push("self binding mismatch");
