...
const EXPECTED_SCOPE_CANONICAL_SHA256 = "3c3c611911db304575c6ecd5736f7610876af90cf5ed6d440059333c890f7ee1";
...
export function validatePlanDocExecutionScope(scope, errors = []) {
  ...
  if (!Array.isArray(records) || records.length !== 75) return [...errors, "historical inventory count mismatch"];
  const key = (record, field) => `${record?.repository}:${record?.[field]}`;
  if (new Set(records.map((record) => key(record, "prNumber"))).size !== 75) errors.push("duplicate historical record identity");
  if (new Set(records.map((record) => key(record, "mergeSha"))).size !== 75) errors.push("duplicate historical merge identity");
  ...
  if (self?.repository !== "AquilaXk/easysubway" || self?.issueNumber !== 2896 || self?.planOwner !== "PLAN-DOC" || self?.changedPathClass !== "HUB_GOVERNANCE_ONLY" || JSON.stringify(self?.allowedChangedFiles) !== JSON.stringify(SELF_FILES)) errors.push("self binding mismatch");
