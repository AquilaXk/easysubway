...
const EXPECTED_SCOPE_CANONICAL_SHA256 = "e084a2f228457b19be78cbb0feb22196eb26c635780a054715736c364782fe23";
...
export function validatePlanDocExecutionScope(scope, errors = []) {
  ...
  if (!Array.isArray(records) || records.length !== 95) return [...errors, "historical inventory count mismatch"];
  const key = (record, field) => `${record?.repository}:${record?.[field]}`;
  if (new Set(records.map((record) => key(record, "prNumber"))).size !== 95) errors.push("duplicate historical record identity");
  if (new Set(records.map((record) => key(record, "mergeSha"))).size !== 95) errors.push("duplicate historical merge identity");
  ...
  if (self?.repository !== "AquilaXk/easysubway" || self?.issueNumber !== 2932 || self?.planOwner !== "PLAN-DOC" || self?.changedPathClass !== "HUB_GOVERNANCE_ONLY" || JSON.stringify(self?.allowedChangedFiles) !== JSON.stringify(SELF_FILES)) errors.push("self binding mismatch");
