...
const EXPECTED_SCOPE_CANONICAL_SHA256 = "54e11473f3e6073188e24807219b65afa7d24b6e2fcd3601386f489e386e24c6";
...
export function validatePlanDocExecutionScope(scope, errors = []) {
  ...
  if (!Array.isArray(records) || records.length !== 91) return [...errors, "historical inventory count mismatch"];
  const key = (record, field) => `${record?.repository}:${record?.[field]}`;
  if (new Set(records.map((record) => key(record, "prNumber"))).size !== 91) errors.push("duplicate historical record identity");
  if (new Set(records.map((record) => key(record, "mergeSha"))).size !== 91) errors.push("duplicate historical merge identity");
  ...
  if (self?.repository !== "AquilaXk/easysubway" || self?.issueNumber !== 2916 || self?.planOwner !== "PLAN-DOC" || self?.changedPathClass !== "HUB_GOVERNANCE_ONLY" || JSON.stringify(self?.allowedChangedFiles) !== JSON.stringify(SELF_FILES)) errors.push("self binding mismatch");
