...
const EXPECTED_SCOPE_CANONICAL_SHA256 = "d3eb61bedf6bf7b183f94cd127c5b817c3bf0a3e572281bbb612aac87b6148e9";
...
export function validatePlanDocExecutionScope(scope, errors = []) {
  ...
  if (!Array.isArray(records) || records.length !== 113) return [...errors, "historical inventory count mismatch"];
  const key = (record, field) => `${record?.repository}:${record?.[field]}`;
  if (new Set(records.map((record) => key(record, "prNumber"))).size !== 113) errors.push("duplicate historical record identity");
  if (new Set(records.map((record) => key(record, "mergeSha"))).size !== 113) errors.push("duplicate historical merge identity");
  ...
  if (self?.repository !== "AquilaXk/easysubway" || self?.issueNumber !== 2957 || self?.planOwner !== "PLAN-DOC" || self?.changedPathClass !== "HUB_GOVERNANCE_ONLY" || JSON.stringify(self?.allowedChangedFiles) !== JSON.stringify(SELF_FILES)) errors.push("self binding mismatch");
