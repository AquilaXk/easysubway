...
function validateClosedResources() {
  ...
  for (const [resourceId, sourcePath] of resources) {
    if (!safePath(resourceId) || !safePath(sourcePath) || resourceIds.has(resourceId) || sourcePaths.has(sourcePath)) {
...
export async function buildDataContractBundle({ repositoryRoot }) {
  ...
  const root = path.resolve(repositoryRoot);
  const entries = await Promise.all(resources.map(async ([resourceId, sourcePath]) => [resourceId, await readRegularFile(root, sourcePath)]));
...
async function main() {
  ...
  if (mode === "--check") {
    const existing = await readRegularFile(root, bundlePath);
