...
export async function collectPublicMetadata({ repository, execGh = boundedGh }) {
  ...
  for (const release of releases) for (const [field, text] of [["name", release.raw.name], ["tag", release.raw.tag_name], ["body", release.raw.body]]) items.push(publicItem({ repository, surface: "RELEASE_METADATA", id: `${release.raw.id}:${field}`, revision: release.raw.published_at, text }));
...
export async function auditPublicSensitivity({ scope, receipts, observedAt, execGh = boundedGh, sourceBytes = {} }) {
  ...
  for (const repository of REPOSITORIES) {
    ...
    if (first.gitSha !== last.gitSha || beginDigest !== endDigest) incomplete.push(incompleteEntry("watermark", "WATERMARK_DRIFT", repository));
  ...
  for (const entry of scope?.falsePositiveDispositions ?? []) if (entry?.verifiedAt > observedAt || entry?.expiresAt <= observedAt || !dispositionKeys.has(`${entry?.locationFingerprint}\u0000${entry?.detectorId}`)) incomplete.push(incompleteEntry("disposition", "INVALID_FALSE_POSITIVE_DISPOSITION", safeFingerprint(entry?.locationFingerprint)));
...
function disposed(finding, entries, observedAt) { return entries.some((entry) => entry.locationFingerprint === finding.locationFingerprint && entry.detectorId === finding.detectorId && entry.verifiedAt <= observedAt && entry.expiresAt > observedAt); }
