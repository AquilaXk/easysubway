...
export function readSingleReceiptZip(bytes) {
  try {
    ...
    if ((flags & ~0x0808) !== 0 || ![0, 8].includes(method) || compressed > ARCHIVE_LIMIT || size > OUTPUT_LIMIT || centralOffset + 46 + nameLength + extraLength + commentLength !== eocd || Math.floor(external / 65_536) >> 12 === 0xa || localOffset + 30 > centralOffset || bytes.readUInt32LE(localOffset) !== 0x04034b50) throw new Error();
    ...
    const localIdentityMatches = usesDataDescriptor
      ? localCrc === 0 && localCompressed === 0 && localSize === 0
      : localCrc === crc && localCompressed === compressed && localSize === size;
    if (name !== RECEIPT_NAME || !localName.equals(centralName) || bytes.readUInt16LE(localOffset + 6) !== flags || bytes.readUInt16LE(localOffset + 8) !== method || !localIdentityMatches) throw new Error();
    ...
    const descriptorIdentityValid = usesDataDescriptor
    ...
    if (data.length !== compressed || !descriptorIdentityValid) throw new Error();
