...
test("plan-doc execution audit scope fixes the federated 89-record inventory and self binding", () => {
  ...
  assert.equal(new Set(SCOPE.historical.map((record) => `${record.repository}:${record.prNumber}`)).size, 89);
  ...
  assert.deepEqual([finalReport.status, finalReport.summary.records, finalReport.summary.findings, finalReport.summary.incomplete], ["COMPLETE", 90, 0, 0]);
