...
test("plan-doc execution audit scope fixes the federated 85-record inventory and self binding", () => {
  assert.equal(SCOPE.schemaVersion, 2);
  assert.equal(SCOPE.historical.length, 85);
  ...
  assert.equal(SCOPE.self.issueNumber, 2910);
  ...
  assert.equal(new Set(SCOPE.historical.map((record) => `${record.repository}:${record.prNumber}`)).size, 85);
  assert.equal(new Set(SCOPE.historical.map((record) => `${record.repository}:${record.mergeSha}`)).size, 85);
  ...
  assert.deepEqual([finalReport.status, finalReport.summary.records, finalReport.summary.findings, finalReport.summary.incomplete], ["COMPLETE", 86, 0, 0]);
