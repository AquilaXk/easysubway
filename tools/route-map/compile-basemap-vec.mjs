...
export function normalizeSvgForCompile(svgText) {
  ...
  const baselineScaled = normalizeTextBaselineAndScale(inlined, k).replaceAll(
    ` ${TERMINAL_CHIP_FONT_EXEMPT_ATTR}="true"`,
    "",
  );
  // 텍스트 위치 완결화는 **마지막**이다 — foldTerminalChipScale·
  // normalizeTextBaselineAndScale이 이미 `<text>` y를 최종값으로 옮겨 놓은 뒤라야
  // 그 값에서 파생한 tspan y가 부모와 같은 기준선을 갖는다.
  return completePartialTextPositions(baselineScaled);
}
