...
export function validatePostGoBoundaryAuditReportSchema(schema, errors = [], path = "post-go-boundary-audit-report.schema") {
  ...
  if (p?.lanes?.minItems !== 2 || p?.lanes?.maxItems !== 2 || JSON.stringify(p?.lanes?.items?.required) !== JSON.stringify(["parent", "status", "declaredJitChildren"]) || JSON.stringify(p?.lanes?.items?.properties?.status?.enum) !== JSON.stringify(["START_BLOCKED", "NOT_PROVEN"]) || p?.findings?.uniqueItems !== true || p?.incomplete?.uniqueItems !== true || !Array.isArray(schema?.oneOf) || schema.oneOf.length !== 2 || schema.oneOf[0]?.properties?.incomplete?.maxItems !== 0 || schema.oneOf[1]?.properties?.incomplete?.minItems !== 1) errors.push(`${path}: fail-closed report parity가 필요하다`);
  return errors;
}
