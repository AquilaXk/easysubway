...
test("plan-doc execution audit contracts fix the 107-record historical inventory and fail-closed report", () => {
  ...
  assert.equal(scope.historical.length, 107);
  ...
  assert.equal(scope.self.issueNumber, 2949);
  ...
  assert.equal(recordsByIdentity.size, 107);
  assert.equal(new Set(scope.historical.map((record) => `${record.repository}:${record.mergeSha}`)).size, 107);
