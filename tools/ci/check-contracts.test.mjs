...
test("plan-doc execution audit contracts fix the 89-record historical inventory and fail-closed report", () => {
  ...
  assert.equal(scope.historical.length, 89);
  ...
  assert.equal(scope.self.issueNumber, 2914);
  ...
  assert.equal(recordsByIdentity.size, 89);
  assert.equal(new Set(scope.historical.map((record) => `${record.repository}:${record.mergeSha}`)).size, 89);
