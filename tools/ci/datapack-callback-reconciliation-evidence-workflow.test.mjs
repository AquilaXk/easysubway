...
test("callback reconciliation evidence는 default branch 수동 실행과 두 identity 입력만 허용한다", () => {
...
test("pinned Node 24·Java 21과 동일 SHA의 #2056 final manifest를 사용한다", () => {
...
test("result 검증 뒤 #2056 gate envelope만 sanitized JSON artifact로 보존한다", () => {
  ...
  assert.doesNotMatch(workflow, /\.env|upload-artifact[\s\S]*?backend\/build\/test-results/);
});
