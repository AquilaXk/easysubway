...
test("백엔드 SSH 배포 스크립트는 상태, drift, 백업, readiness 롤백 계약을 포함한다", async () => {
  ...
  assert.match(deploy, /OBSERVABILITY_STABLE_SERVICES=\(public-edge-probe docker-runtime-probe prometheus loki grafana\)/);
  ...
  assert.match(deploy, /--profile observability up -d --no-build "\$\{OBSERVABILITY_STABLE_SERVICES\[@\]\}"/);
  ...
  assert.match(deploy, /start_observability_services "\$\{SHARED_DIR\}\/current-env\/backend\.env" "\$\{SHARED_DIR\}\/current-env\/compose\.env" "\$\{current_sha\}" "\$\{recreate_alertmanager\}" \|\| true/);
