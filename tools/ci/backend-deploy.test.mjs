...
test("백엔드 SSH 배포 스크립트는 상태, drift, 백업, readiness 롤백 계약을 포함한다", async () => {
  ...
  assert.match(deploy, /systemctl stop "\$\{legacy_unit\}"/);
  assert.match(deploy, /systemctl disable "\$\{legacy_unit\}"/);
  ...
  assert.ok(deploy.indexOf("stop_legacy_backend_service") < deploy.indexOf("backend_id="));
  ...
  assert.match(deploy, /rm -f -s backend/);
  assert.match(deploy, /if ! compose "\$\{SHARED_DIR\}\/current-env\/backend\.env" "\$\{SHARED_DIR\}\/current-env\/compose\.env" "\$\{DEPLOY_SHA\}" up -d --no-deps --no-build backend; then/);
