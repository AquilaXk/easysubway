...
test("운영 관측성과 알림 기준선은 필수 release 신호와 심볼 보관 계약을 고정한다", () => {
  ...
  assert.match(datapackWorkflow, /remotePublishEnabled === "true" && remotePublishReady !== "true"/);
  assert.match(datapackWorkflow, /publishResult = remotePublishResult \|\| "blocked-strict-coverage"/);
...
test("모바일 스캐폴드는 Flutter Android와 iOS 앱 구조를 가진다", () => {
  ...
  assert.match(main, /내 정보 삭제 요청 시 즐겨찾기, 이동 조건, 제보 접수 기록, 제보 내용·사진·위치와 경로 피드백을 삭제하거나 개인을 알 수 없게 처리합니다/);
