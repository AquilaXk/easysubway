...
test("지속적 통합 작업과 스텝 이름은 실패 영역을 구분할 수 있게 표시된다", () => {
  ...
  assert.match(workflow, /description: Run iOS CI on macOS only when explicitly requested/);
...
test("필수 지속적 통합 작업은 변경 없는 영역도 성공 상태로 종료한다", () => {
  ...
  assert.match(
    iosJob,
    /if: \$\{\{ github\.event_name == 'workflow_dispatch' && inputs\.include_ios == true && needs\.changes\.outputs\.ios == 'true' \}\}/,
  );
});
...
test("릴리즈 산출물 워크플로우는 모바일 스토어 산출물과 backend image 검증을 생성한다", () => {
  ...
  assert.match(workflow, /workflow_dispatch:/);
  assert.match(workflow, /workflow_dispatch:\s*\n\s*inputs:\s*\n\s*include_ios:/);
  assert.match(workflow, /description: Build iOS release artifact only when explicitly requested/);
  assert.match(workflow, /default: false/);
  assert.match(workflow, /type: boolean/);
  assert.match(workflow, /permissions:\s*\n\s*contents: read/);
...
test("릴리즈 산출물 워크플로우는 관련 변경에서만 비용 큰 산출물 빌드를 실행한다", async () => {
  ...
  assert.match(
    iosReleaseJob,
    /if: \$\{\{ github\.event_name == 'workflow_dispatch' && inputs\.include_ios == true && \(needs\.changes\.outputs\.ios == 'true' \|\| needs\.changes\.outputs\.mobile == 'true'\) \}\}/,
  );
...
test("운영 데이터팩 공식 출처 inventory는 라이선스와 갱신 기준을 기계 검증 가능하게 고정한다", () => {
  ...
  for (const source of inventory.sources) {
    ...
    assert.equal(source.license.type, "KOGL-1", `${source.id} must use an explicit public data license type`);
    ...
    assert.match(source.license.attribution, /공공누리 제1유형/);
    assert.match(source.datasetUrl, /^https:\/\/data\.seoul\.go\.kr\/dataList\/OA-[0-9]+\/[FS]\/1\/datasetView\.do$/);
