...
test("데이터팩 workflow는 pack 검증 이후 manifest 배포 순서를 강제한다", () => {
  ...
  assert.ok(
    workflow.indexOf("--require-production", remotePublishIndex) < workflow.indexOf("publish-object-storage.mjs", remotePublishIndex),
...
test("백엔드 데이터 품질 요약은 관리자 API와 헥사고날 경계를 따른다", () => {
  ...
  assert.match(mobileStationSearch, /'LEVEL_1' => '기본 정보만 있음'/);
  assert.match(mobileStationSearch, /'LEVEL_2' => '시설 정보 확인됨'/);
  assert.match(mobileStationSearch, /'LEVEL_3' => '쉬운 길 안내 가능'/);
  assert.match(mobileStationSearch, /'LEVEL_4' => '고장·공사 반영됨'/);
