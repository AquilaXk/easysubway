...
test("백엔드 데이터 수집 배치는 관리자 API와 Spring Batch 경계를 따른다", () => {
  ...
  assert.match(buildGradle, /testRuntimeOnly 'com\.h2database:h2'/);
  assert.doesNotMatch(buildGradle, /runtimeOnly 'com\.h2database:h2'/);
  ...
  assert.match(applicationProd, /sql:[\s\S]*init:[\s\S]*mode: always/);
  ...
  assert.doesNotMatch(applicationProd, /EASYSUBWAY_DATASOURCE_/);
