...
test("Docker Compose는 backend 필수 서비스와 관측성 선택 서비스를 profile로 분리한다", () => {
  const compose = read("infra/docker-compose.yml");

  assert.match(compose, /postgres:\n/);
  assert.match(compose, /image: postgis\/postgis:16-3\.5/);
  assert.match(compose, /postgres:[\s\S]*profiles:\s*\n\s*-\s*backend/);
  ...
  assert.match(compose, /object-storage:[\s\S]*profiles:\s*\n\s*-\s*backend/);
