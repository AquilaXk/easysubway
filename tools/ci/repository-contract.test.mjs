...
test("지속적 통합 작업과 스텝 이름은 실패 영역을 구분할 수 있게 표시된다", () => {
  ...
  assert.match(releaseGateJob, /node --test tools\/ci\/check-map-attribution\.test\.mjs/);
...
test("CD dotenv 검증은 운영 fallback env 계약을 반영한다", async () => {
  ...
  await writeFile(envFile, [
    ...
    "EASYSUBWAY_ALERTMANAGER_EXTERNAL_URL=https://prometheus/alerts",
...
test("모바일 오프라인 안내 화면(OfflineDataScreen)은 완전히 제거됐다 (#1570)", () => {
  ...
  assert.deepEqual(finalQaEvidence.remainingChecklistClosed, [
    "offline_metadata_missing_state",
    "manual_accessibility_qa_scenarios",
    "text_scale_200_screenshot_evidence",
    "text_scale_200_screenshot_test",
  ]);
  assert.ok(finalQaEvidence.manualQaScenarios.some((item) => item.platform === "android" && item.evidence.includes("uiautomator")));
  assert.ok(finalQaEvidence.manualQaScenarios.some((item) => item.platform === "ios" && item.evidence.includes("xcodebuildmcp")));
});
...
test("모바일 signed release artifact gate와 광고 counter는 CI 산출물과 스토어 제출 준비 상태를 분리한다", () => {
  ...
  assert.ok(gate.artifacts.android.storeReadyRequires.includes("Phase A pre-launch operations readiness and dry-run evidence"));
  assert.equal(gate.operationsPhasePolicy.preLaunchRequiredStatus, "PASS");
  assert.equal(gate.operationsPhasePolicy.prePublicPostLaunchStatus, "PENDING_PUBLIC_RELEASE");
  assert.equal(gate.operationsPhasePolicy.actualPostLaunchObservationsRequiredBeforeFirstPublicRelease, false);
  assert.ok(
  ...
  assert.equal(
    postLaunchOperationsReviewGate.postLaunchObservation.dueTimePolicy,
    "dueAt = publicReleaseIdentity.publishedAt + reviewWindows[].afterPublicRelease",
  );
  assert.deepEqual(postLaunchOperationsReviewGate.postLaunchObservation.allowedStatusTransitions, [
    "PENDING_PUBLIC_RELEASE->IN_PROGRESS",
    "IN_PROGRESS->PASS",
  ]);
  assert.ok(
    postLaunchOperationsReviewGate.preLaunchReadiness.finalRcBinding.requiredFields.includes("gitSha"),
  );
  assert.ok(
  ...
  assert.deepEqual(postLaunchOperationsReviewGate.preLaunchReadiness.finalRcBinding.evidenceValidity, {
    ...
    refreshOn: [
      ...
      "observability-infrastructure-change",
      "fixed-release-procedure-change",
    ],
  });
  const refreshBindings = postLaunchOperationsReviewGate.preLaunchReadiness.finalRcBinding.refreshBindings;
  assert.deepEqual(
  ...
  assert.deepEqual(postLaunchOperationsReviewGate.latestQaEvidenceSummary.alertRouteDryRun.severities, ["P0", "P1", "P2"]);
  assert.match(
  ...
  for (const window of postLaunchOperationsReviewGate.reviewWindows) {
    ...
    assert.equal(window.dueAtFormula, `publicReleaseIdentity.publishedAt+${window.afterPublicRelease}`);
    assert.match(window.recordLocation, /^\.codex\/evidence\/release\/post-launch-operations-review\//);
  }
  assert.deepEqual(
  ...
  assert.equal(supportIncidentResponseGate.status, "PASS");
  assert.equal(supportIncidentResponseGate.preLaunchReadiness.status, "PASS");
  assert.equal(supportIncidentResponseGate.latestQaEvidenceSummary.qaEvidenceDateKst, "2026-07-15");
  assert.deepEqual(supportIncidentResponseGate.latestQaEvidenceSummary.remainingSupportReadiness, []);
  assert.ok(
    supportIncidentResponseGate.preLaunchReadiness.resolvedEvidence.includes(
  ...
  assert.equal(supportIncidentResponseGate.latestQaEvidenceSummary.operatorContactReadiness.result, "PASS");
  assert.match(
    supportIncidentResponseGate.latestQaEvidenceSummary.operatorContactReadiness.localOnlyEvidence,
    /operator-contact-readiness\.json$/,
  );
  for (const evidenceId of supportIncidentResponseGate.operatorContactRoutes.flatMap((route) => route.requiredEvidence)) {
    assert.ok(supportIncidentResponseGate.preLaunchReadiness.resolvedEvidence.includes(evidenceId));
  }
...
test("RC evidence manifest generator는 RC identity와 No-Go blocker를 생성한다", async () => {
  ...
  await writeFile(phaseASummaryPath, JSON.stringify({
    ...
    issue: 1019,
    evidenceValidity: {
      testedAt: "2999-01-01T00:00:00Z",
      expiresWhen: "2999-01-15T00:00:00Z",
    },
  }));
  await assert.rejects(
...
`);
  ...
  await writeFile(dataPackManifestPath, JSON.stringify({ manifestVersion: 2, channel: "production", releaseSequence: 102 }));
  ...
  await writeFile(identityLinkagePath, `${JSON.stringify({
    ...
    mobileTopologyPack: { artifactId: "mobile-topology-v1", sha256: "3".repeat(64), freshUntil: "2026-07-30T00:00:00.000Z", identity: sharedIdentity },
  }, null, 2)}\n`);
  ...
  await rejectsCurrent(/rollback_rescue requires result schemaVersion 1/);
  rollbackEvidence.result = datapackGateResult("rollback_rescue");
  rollbackEvidence.result.rescueReleaseSequence += 1;
  await writeFile(gateEvidencePaths.rollback_rescue, JSON.stringify(rollbackEvidence));
  await rejectsCurrent(/rollback_rescue result does not match the RC identity/);
  rollbackEvidence.result = datapackGateResult("rollback_rescue");
  await writeFile(gateEvidencePaths.rollback_rescue, JSON.stringify(rollbackEvidence));
  ...
  await rejectsCurrent(/source_admission requires result schemaVersion 1/);
  ...
  await rejectsCurrent(/device_performance artifact.sha256 does not match the RC identity/);
  ...
  await writeFile(gateEvidencePaths.device_performance, JSON.stringify(devicePerformanceEvidence));
  await rejectsCurrent(/device_performance compressedBytes does not match the supplied data pack artifact/);
  devicePerformanceEvidence.result = datapackGateResult("device_performance");
  devicePerformanceEvidence.result.metrics.coldLoadP95Ms = devicePerformanceEvidence.result.metrics.coldLoadP50Ms - 1;
  await writeFile(gateEvidencePaths.device_performance, JSON.stringify(devicePerformanceEvidence));
  await rejectsCurrent(/requires P95 metrics to be greater than or equal to P50 metrics/);
  devicePerformanceEvidence.result = datapackGateResult("device_performance");
  await writeFile(gateEvidencePaths.device_performance, JSON.stringify(devicePerformanceEvidence));
  ...
  await rejectsCurrent(/callback_reconciliation releaseSequence does not match the RC identity/);
  ...
  await writeFile(gateEvidencePaths.callback_reconciliation, JSON.stringify(callbackEvidence));
  await rejectsCurrent(/callback_reconciliation idempotencyKeySha256 does not match the delivery identity/);
  callbackEvidence.result = datapackGateResult("callback_reconciliation");
  await writeFile(gateEvidencePaths.callback_reconciliation, JSON.stringify(callbackEvidence));
  ...
  await rejectsCurrent(/identity linkage has unsafe or non-scalar mobileTopologyPack.identity/);
  ...
  await writeFile(identityLinkagePath, JSON.stringify(linkageEvidence));
  await rejectsCurrent(/identity linkage shared field mismatch: corridorId/);
  ...
  await writeFile(identityLinkagePath, JSON.stringify(linkageEvidence));
  await rejectsCurrent(/missing or invalid shared field: schemaVersion/);
  for (const artifact of ["sourceTimetableArtifact", "serverTimetableSnapshot", "mobileTopologyPack"]) {
    linkageEvidence[artifact].identity.schemaVersion = 3;
  }
  await writeFile(identityLinkagePath, JSON.stringify(linkageEvidence));
  ...
  await writeFile(identityLinkagePath, JSON.stringify(linkageEvidence));
  await rejectsCurrent(/identity linkage freshness does not cover evidence expiry: mobileTopologyPack/);
  ...
  await rejectsCurrent(/mixed snapshotSetIdentity/);
  ...
  await writeFile(gateEvidencePaths.source_governance, JSON.stringify(governanceEvidence));
  await rejectsCurrent(/include every required production source/);
  ...
  await writeFile(gateEvidencePaths.source_governance, JSON.stringify(governanceEvidence));
  await rejectsCurrent(/sourceInventory.inventoryAsOf must cover child evidence/);
  governanceEvidence.sourceInventory.inventoryAsOf = "2026-07-17T00:00:00.000Z";
  await writeFile(gateEvidencePaths.source_governance, JSON.stringify(governanceEvidence));
  await rejectsCurrent(/requires sourceInventory.inventoryAsOf/);
  ...
  await writeFile(gateEvidencePaths.source_governance, JSON.stringify(governanceEvidence));
  await rejectsCurrent(/invalid, duplicate, stale, or contradictory sourceInventory.entries/);
  ...
  await writeFile(gateEvidencePaths.source_governance, JSON.stringify(governanceEvidence));
  await rejectsCurrent(/statusCounts must match current sourceInventory.entries/);
});
...
test("Android release 100 governance gate는 Android-only 범위와 evidence schema를 고정한다", () => {
  ...
  assert.deepEqual(
    gate.gates.find((item) => item.id === "G14_ROUTE_RELEASE_READINESS"),
    {
...
test("운영 관측성과 알림 기준선은 필수 release 신호와 심볼 보관 계약을 고정한다", () => {
  ...
  assert.equal(
    ...
    "node tools/ops/validate-operations-release-summary.mjs --summary <summary.json> --rc-manifest <rc-evidence-manifest.json> --require-pass",
  );
...
test("데이터팩 workflow는 pack 검증 이후 manifest 배포 순서를 강제한다", () => {
  ...
  const publicCatalogUserVersionGateIndex = workflow.indexOf(
  ...
  assert.notStrictEqual(
    publicCatalogUserVersionGateIndex,
    -1,
    "workflow must enforce public catalog user_version compatibility before remote publish",
  );
  assert.ok(
  ...
  assert.ok(
    publicCatalogUserVersionGateIndex < publishObjectStorageIndex,
    "workflow must reject incompatible catalog user_version before remote publish",
  );
...
test("KRIC source 후보는 상세 근거 완료 상태와 production 분리를 고정한다", () => {
  ...
  for (const candidate of kricCandidates) {
    ...
    assert.equal(
      candidate.sampleEvidenceStatus,
...
test("KRIC 출입구 승강장 이동경로 후보는 상세 근거가 있어도 route graph edge로 자동 승격하지 않는다", () => {
  ...
  const source = inventory.sources.find(({ id }) => id === candidate.id);
...
test("KRIC 환승 이동경로 표준 후보는 상세 페이지 라이선스와 출력변수 근거를 기록한다", () => {
  ...
  assert.equal(candidate.evidence.adminReview.decision, "REJECTED_NO_DATA");
  assert.match(candidate.evidence.adminReview.reasonKo, /29251592781/);
  assert.match(candidate.evidence.adminReview.reasonKo, /29252661883/);
  assert.match(candidate.evidence.adminReview.reasonKo, /resultCode=03/);
  assert.match(candidate.nextAction, /no-data.*admission을 거부.*row-bearing.*재심사.*automaticRouteGraphEdgeAllowed=false/);
});
...
test("KRIC 도시철도 전체노선정보 후보는 상세 페이지 라이선스와 출력변수 근거를 기록한다", () => {
  ...
  assert.equal(candidate.mobileEmbeddingAllowed, false);
  assert.equal(candidate.automaticRouteGraphEdgeAllowed, false);
  assert.equal(inventory.sources.some(({ id }) => id === candidate.id), true);
  ...
  assert.equal(candidate.evidence.liveSampleEvidenceHash, "bf0d324018c6d2353bfe4da8c7385ef867aacb27a26dcda311a259641a35aa03");
  assert.deepEqual(candidate.evidence.liveSampleFields.sort(), candidate.evidence.outputFields.sort());
  assert.match(
    candidate.evidence.liveSampleNote,
    /29148603270 \(head 6d8e4f8f3fc8398f981555e99bb37ba34e7afcd1\).*backend-only.*credentialRedacted=true.*format=xml.*rowCount=14.*sample↔hashes.*validator.*raw\/provider-record 독립 재계산은 수행하지 않음.*mobile\/offline pack에 포함하지 않음.*mreaWideCd=01.*lnCd=A1.*4호선 coverage가 아님/,
  );
  ...
  assert.match(candidate.nextAction, /admin review.*inventory provenance.*#1400.*productionUseAllowed=false/);
});
...
test("KRIC 역사별 정보 후보는 상세 페이지 라이선스와 출력변수 근거를 기록한다", () => {
  ...
  assert.equal(sampleUrl.searchParams.get("stinCd"), "135");
  assert.equal(sampleUrl.searchParams.get("stinNm"), "용산");
  assert.deepEqual(candidate.evidence.formats.sort(), ["JSON", "XML"]);
  assert.deepEqual(candidate.evidence.outputFields.sort(), [
    "lnCd",
    "lonmAdr",
  ...
  assert.equal(candidate.evidence.liveSampleEvidenceHash, "cf5b877a765f5d84b48450b6aa5ef5fe9fea5776ca856dba7dafccada9a6d2de");
  assert.deepEqual(candidate.evidence.liveSampleFields.sort(), candidate.evidence.outputFields.sort());
  assert.match(
    candidate.evidence.liveSampleNote,
    /29148721533 \(head 6d8e4f8f3fc8398f981555e99bb37ba34e7afcd1\).*backend-only.*credentialRedacted=true.*format=xml.*rowCount=1.*sample↔hashes.*validator.*raw\/provider-record 독립 재계산은 수행하지 않음.*mobile\/offline pack에 포함하지 않음.*railOprIsttCd=KR.*lnCd=1.*stinCd=135.*stinNm=용산.*4호선 coverage가 아님/,
  );
  assert.deepEqual(candidate.evidence.missingEvidence.sort(), [
    "credentialFreeRawArchive",
...
test("백엔드는 모바일 기본 경로용 public transit, me API와 search 외 route API를 노출하지 않는다", () => {
  ...
  const routeSearchController = read("backend/src/main/java/com/easysubway/route/adapter/in/web/RouteSearchController.java");
  ...
  assert.match(routeSearchController, /@PostMapping\("\/api\/v1\/routes\/search"\)/);
  assert.doesNotMatch(backendSources, /@(?:Get|Post|Put|Patch|Delete)Mapping\("\/api\/v1\/routes\/(?!search"\))/);
...
test("신고 조회와 경로 피드백 권한 경계는 인증 사용자 기준이다", () => {
  ...
  const routeController = read(routeControllerPath);
  ...
  assert.doesNotMatch(routeController, /feedback|SubmitRouteFeedbackCommand/);
});
