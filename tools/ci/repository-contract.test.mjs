...
test("시설 신고 사진 백업은 로컬 전용 객체와 manifest 기준선을 제공한다", () => {
  ...
  assert.match(backupScript, /ENCODE\(CONVERT_TO\(COALESCE\(photo_file_name, ''\), 'UTF8'\), 'base64'\)/);
  assert.match(backupScript, /ENCODE\(CONVERT_TO\(COALESCE\(photo_content_type, ''\), 'UTF8'\), 'base64'\)/);
