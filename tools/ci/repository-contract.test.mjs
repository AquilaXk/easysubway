...
test("OCI Terraform 기준선은 비밀 파일을 추적하지 않고 데이터팩 출력 계약을 제공한다", () => {
  ...
  const trackedInfraFiles = execFileSync("git", ["ls-files", "--cached", "--others", "--exclude-standard", terraformDir], {
    cwd: root,
    encoding: "utf8",
  }).trim().split("\n").filter(Boolean);
  ...
  assert.equal(
    trackedInfraFiles.some((file) => /\.(tfstate|tfplan|pem)$/.test(file) || file.includes("/.terraform/")),
    false,
  ...
  assert.match(tfvarsExample, /datapack\.aquilaxk\.site\/datapacks/);
