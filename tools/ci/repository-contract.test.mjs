...
test("데이터팩 workflow는 pack 검증 이후 manifest 배포 순서를 강제한다", () => {
  ...
  assert.ok(preflightIndex > verifyIndex, "workflow must create publish preflight plan after pack checksum verification");
  assert.ok(manifestIndex > preflightIndex, "workflow must stage manifest after publish preflight plan");
