...
test("모바일 홈 shell과 주요 상태 UI 회귀 테스트는 유지된다", () => {
  ...
  assert.match(widgetTest, /노선도 메뉴 길찾기는 길찾기 화면으로 전환한다/);
...
test("4호선 상록수-사당 topology 후보는 production scope와 분리한다", () => {
  ...
  assert.deepEqual(
    candidate.supportBoundary.candidateOnlyStationIds.toSorted(),
    candidate.stations
      .map((station) => station.stationId)
      .filter((stationId) => !productionInput.supportedV1Scope.includedStationIds.includes(stationId))
      .toSorted(),
  );
  assert.equal(candidate.stations.length, candidate.candidateGraph.stationCount);
  assert.equal(candidate.candidateGraph.stationCount, 16);
  assert.equal(candidate.candidateGraph.undirectedSegmentCount, 15);
  assert.equal(candidate.candidateGraph.directedRideEdgeCandidateCount, 30);
  assert.deepEqual(
    candidate.stations.map((station) => station.sequence),
    Array.from({ length: 16 }, (_, index) => 28 + index),
  );
  assert.deepEqual(
    candidate.stations.map((station) => station.stationNameKo),
    [
      "사당",
      "남태령",
      "선바위",
  ...
  for (const station of candidate.stations) {
    assert.equal(typeof station.routeMapX, "number");
    assert.equal(typeof station.routeMapY, "number");
    assert.ok(station.routeMapX >= 0);
    assert.ok(station.routeMapY >= 0);
    assert.equal(station.stationNameKo, station.routeMapSourceLabel);
  }
  assert.deepEqual(candidate.candidateGraph.confirmedFields.toSorted(), [
    "line_sequence_adjacency",
    "route_map_position",
    "station_line_membership",
  ]);
  assert.deepEqual(candidate.candidateGraph.missingConfirmedFields.toSorted(), [
    "accessibility_facilities",
    "distance_meters",
    "duration_seconds",
    "field_or_operator_pathway_review",
  ]);
  assert.equal(candidate.sourceEvidence.routeMapPositionSourceId, "seoulmetro-cyberstation-route-map");
  assert.equal(
    candidate.sourceEvidence.routeMapSourceSha256,
    "7370b4db2d2f398f46c55314b71d7335c77ec6745fd388793804874447cd25e0",
  );
  assert.deepEqual(candidate.sourceEvidence.stationLineMembershipSourceIds, [
    "molit-urban-rail-full-route",
    "seoulmetro-station-line-info",
  ]);
  for (const sourceId of [
    ...candidate.sourceEvidence.stationLineMembershipSourceIds,
    candidate.sourceEvidence.routeMapPositionSourceId,
  ]) {
    assert.ok(inventorySources.has(sourceId), `${sourceId} must exist in source inventory`);
  }
});
