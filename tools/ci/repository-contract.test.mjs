...
test("시설 신고 사진 백업은 로컬 전용 객체와 manifest 기준선을 제공한다", () => {
  ...
  assert.match(backupScript, /EASYSUBWAY_REPORT_PHOTO_STORAGE_DIR:-\$\{ROOT_DIR\}\/\.codex\/report-photos/);
  assert.match(backupScript, /umask 077/);
  assert.match(backupScript, /chmod 700 "\$\{BACKUP_DIR\}"/);
...
test("백엔드 시설 신고는 헥사고날 API 경계를 따른다", () => {
  ...
  assert.doesNotMatch(jdbcRepository, /photo_data_base64/);
  ...
  assert.match(photoObjectMigration, /DROP COLUMN IF EXISTS photo_data_base64/);
