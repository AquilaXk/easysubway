...
test("KRIC 부분확인 source 후보는 production inventory와 분리한다", () => {
  const inventory = readJson("tools/datapack/source-inventory.json");
...
test("KRIC 환승 이동경로 표준 후보는 샘플 근거만 기록하고 라이선스 partial을 유지한다", () => {
  const candidates = readJson("tools/datapack/source-candidates.json");
...
test("KRIC 출입구 승강장 이동경로 표준 후보는 상세 페이지 라이선스와 출력변수 근거를 기록한다", () => {
  ...
  assert.deepEqual(candidate.evidence.missingEvidence.sort(), ["distanceMeters", "durationSeconds", "sampleResponse"]);
});
...
test("KRIC 역사별 환승정보 후보는 샘플 근거만 기록하고 라이선스 partial을 유지한다", () => {
  const candidates = readJson("tools/datapack/source-candidates.json");
