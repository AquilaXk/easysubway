...
test("백엔드 데이터 수집 배치는 관리자 API와 Spring Batch 경계를 따른다", () => {
  ...
  assert.match(batchPostgresSchema, /CREATE TABLE IF NOT EXISTS data_collection_run_steps/);
  assert.match(batchPostgresSchema, /step_name VARCHAR\(40\) NOT NULL/);
  assert.match(batchPostgresSchema, /artifact_reference VARCHAR\(1000\)/);
  assert.match(batchPostgresSchema, /checksum VARCHAR\(64\)/);
  ...
  assert.match(batchConfig, /FETCH_STEP_NAME/);
  assert.match(batchConfig, /ARCHIVE_STEP_NAME/);
  assert.match(batchConfig, /VALIDATE_STEP_NAME/);
  assert.match(batchConfig, /PARSE_STEP_NAME/);
  assert.match(batchConfig, /DIFF_STEP_NAME/);
  assert.match(batchConfig, /STAGE_STEP_NAME/);
