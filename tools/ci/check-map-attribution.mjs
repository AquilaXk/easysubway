...
const licenseDecisionPath = path.join(repoRoot, "tools/route-map/route-map-license-decision.json");
const allowedReviewStatus = existsSync(licenseDecisionPath)
  ? JSON.parse(readFileSync(licenseDecisionPath, "utf8")).reviewStatusValues
  : ["APPROVED", "REVIEW_REQUIRED", "BLOCKED", "EXPIRED", "UNREVIEWED"];
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const manifestPath = path.join(repoRoot, "apps/mobile/assets/datapacks/metro_map_pack/manifest.json");
  if (!existsSync(manifestPath)) {
    console.log("map attribution ok (manifest not present; skipping)");
    process.exit(0);
  }
