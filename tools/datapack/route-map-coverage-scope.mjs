...
function collectScopeCoverage({ inventory, snapshotsByPath, violations }) {
  ...
  for (const source of inventory.sources ?? []) {
    ...
    if (!scope?.sourceDomains?.includes(ROUTE_MAP_DOMAIN)) {
      continue;
    }
    const lineIds = scope.lineIds ?? [];
    if (lineIds.length === 0) {
    ...
    for (const regionId of scope.regionIds ?? []) {
      for (const operatorId of scope.operatorIds ?? []) {
        for (const lineId of lineIds) {
          ...
          claims.push({ sourceId: source.id, key, lineId });
...
function validateAliases({ aliases, auditedScopeKeys, coverageByScope, rosters, violations }) {
  ...
  for (const [index, alias] of aliases.entries()) {
    ...
    const push = (kind, message) => violations.push({ kind, scopeKey: alias?.scopeKey, message: `${label}: ${message}` });
    ...
    const position = coverage.positionsByName.get(snapshotName);
    ...
    if (!validateAliasReason({ alias, coverage, roster, position, snapshotName, rosterName, push })) {
      continue;
    }
    seenSnapshotNames.add(snapshotKey);
    seenRosterNames.add(rosterKey);
...
function validateAliasReason({ alias, coverage, roster, position, snapshotName, rosterName, push }) {
  switch (alias.reasonCode) {
    case "OFFICIAL_LINE_ORDINAL_SUFFIX": {
      // 대구 공식 파일·pack topology는 환승역을 "역명 + 호선번호"로 표기한다(명덕1 = 1호선 명덕).
      const ordinal = position.line;
...
function validateGaps({
  ...
  for (const [index, gap] of gaps.entries()) {
    ...
    const push = (kind, message) => violations.push({ kind, scopeKey: gap?.scopeKey, message: `${label}: ${message}` });
    ...
    if (coverage.positionsByName.has(rosterName)
      || aliasedRosterNamesByScope.get(gap.scopeKey)?.has(rosterName)) {
    ...
    if (!isNonEmptyString(evidence.snapshotPath) || !coverage.snapshotPaths.includes(evidence.snapshotPath)) {
      push("LEDGER_SNAPSHOT_NOT_CLAIMED", "evidence.snapshotPath가 이 scope를 커버하는 admitted snapshot이 아니다");
      continue;
    }
    if (!validateGapReason({
      gap,
      coverage,
      rosterName,
      snapshot: snapshotsByPath.get(evidence.snapshotPath),
      topologiesByPath,
      push,
    })) {
      continue;
    ...
    seen.add(key);
    const ledgered = gapRosterNamesByScope.get(gap.scopeKey) ?? new Set();
    ledgered.add(rosterName);
    gapRosterNamesByScope.set(gap.scopeKey, ledgered);
  }
  return gapRosterNamesByScope;
}

function validateGapReason({ gap, coverage, rosterName, snapshot, topologiesByPath, push }) {
  const evidence = gap.evidence;
  switch (gap.reasonCode) {
    case "ADMISSION_QUARANTINED": {
      const quarantined = coverage.quarantinedByName.get(rosterName);
      if (!quarantined || !isNonEmptyString(evidence.quarantineReasonCode)
        || quarantined.reasonCode !== evidence.quarantineReasonCode) {
        push("LEDGER_QUARANTINE_RECORD_ABSENT", "snapshot quarantinedPositions에 같은 reasonCode 기록이 없다");
        return false;
      }
      return true;
    }
    case "OFFICIAL_FILE_ROW_ABSENT": {
      if (!isHttpsUrl(evidence.officialUrl)) {
        push("LEDGER_EVIDENCE_INVALID", "evidence.officialUrl 공식 원문 출처가 필요하다");
        return false;
      }
      const topologyNames = resolveGapTopologyNames({ evidence, coverage, topologiesByPath, push });
      if (!topologyNames) {
        return false;
      }
    ...
    case "PACK_SCOPE_ABSENT": {
      ...
      // pack topology 자체가 역을 싣지 않은 경우만 이 사유에 해당한다.
      if (topologyNames.has(rosterName)) {
        push("LEDGER_PACK_TOPOLOGY_STATION_PRESENT", "pack topology에 있는 역은 PACK_SCOPE_ABSENT가 아니다");
        return false;
      }
      return true;
    }
...
function resolveGapTopologyNames({ evidence, coverage, topologiesByPath, push }) {
  if (!isNonEmptyString(evidence.packTopologyPath)) {
    push("LEDGER_PACK_TOPOLOGY_MISSING", "evidence.packTopologyPath가 필요하다");
    return null;
  }
  const topology = topologiesByPath.get(evidence.packTopologyPath);
  const stationNames = resolveTopologyStationNames(topology, coverage.lineId);
  if (!stationNames) {
...
export function auditRouteMapCoverageScopes({
  ...
  for (const claim of claims) {
    ...
    if (!activeScopeKeys.has(claim.key) || auditedScopeKeys.includes(claim.key)) {
      continue;
    }
  ...
  return { auditedScopeKeys: auditedScopeKeys.sort(), violations };
}
