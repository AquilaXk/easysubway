...
function parseCli(argv) {
  if (argv.length !== 2 || argv[0] !== "--candidate" || !argv[1] || argv[1].startsWith("--")) {
    throw new Error("usage: collect-datago-source-candidate-evidence.mjs --candidate <fixed-candidate-id>");
  }
...
function datagoXmlDiagnostic(response, requestedFormat, raw) {
...
export async function collectDatagoSourceCandidateEvidence({
  ...
  const outputDirectory = path.join(runnerTemp, "datago-source-candidate-evidence");
  const stagingDirectory = path.join(runnerTemp, "datago-source-candidate-staging");
  const rawDirectory = path.join(runnerTemp, "datago-source-candidate-raw");
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`data.go.kr request failed with HTTP ${response.status}`);
    }
    ...
    try {
      ...
      if (request.format === "xml") {
        throw new Error(`${error instanceof Error ? error.message : String(error)} ${datagoXmlDiagnostic(response, request.format, rawResponse)}`);
      }
      throw error;
    }
    JSON.parse(sample);
