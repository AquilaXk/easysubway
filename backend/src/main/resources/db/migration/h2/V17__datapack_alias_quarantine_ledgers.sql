CREATE TABLE IF NOT EXISTS external_alias_approvals (
	...
	CONSTRAINT chk_external_alias_approvals_evidence_hash
...
CREATE TABLE IF NOT EXISTS source_quarantine_records (
	...
	source_id VARCHAR(120) NOT NULL,
	source_snapshot_id VARCHAR(120) NOT NULL,
	provider_record_hash VARCHAR(64) NOT NULL,
	...
	CONSTRAINT chk_source_quarantine_records_provider_hash
		CHECK (char_length(provider_record_hash) = 64)
);
...
CREATE TABLE IF NOT EXISTS source_quarantine_resolutions (
	...
	CONSTRAINT chk_source_quarantine_resolutions_evidence_hash
		CHECK (evidence_hash IS NULL OR char_length(evidence_hash) = 64)
);
