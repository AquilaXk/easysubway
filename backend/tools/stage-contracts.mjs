...
const artifactUrlPattern = /^https:\/\/raw\.githubusercontent\.com\/AquilaXk\/easysubway\/(?:main|[a-f0-9]{40})\/contracts\/bundles\/backend-contracts-v1\.0\.0\.json(?![\s\S])/;
...
try {
  ...
  if (lockDocument.schemaVersion !== 1 || lockDocument.bundleVersion !== "1.0.0" || typeof lockDocument.artifactUrl !== "string" || !artifactUrlPattern.test(lockDocument.artifactUrl) || !/^[a-f0-9]{64}$/.test(lockDocument.sha256)) {
